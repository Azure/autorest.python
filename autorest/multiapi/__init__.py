...
class MultiAPI:
    ...
    def _build_last_rt_list(
        ...
        for operation, api_versions_list in versioned_dict.items():
            local_last_api_version = MultiAPI._get_floating_latest(api_versions_list, preview_mode)
    ...
    def process(self) -> bool:
        ...
        path_to_package = (
            sdk_root / MultiAPI._resolve_package_directory(package_name, sdk_root) /
            Path(*(module_name.split(".")))
        ).resolve()
        paths_to_versions = MultiAPI._get_paths_to_versions(path_to_package)
        versioned_operations_dict, mod_to_api_version = MultiAPI._build_operation_meta(
            paths_to_versions
        )

        last_api_version = MultiAPI._get_floating_latest(mod_to_api_version.keys(), preview_mode)
        ...
        mixin_operations = MultiAPI._build_operation_mixin_meta(paths_to_versions)
        ...
        last_rt_list = MultiAPI._build_last_rt_list(
