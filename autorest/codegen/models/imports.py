...
class FileImport:
    def __init__(self) -> None:
        ...
        self._imports: Dict[bool, Dict[ImportType, Dict[str, Set[Optional[str]]]]] = dict()
    ...
    def add_from_import(
        ...
        """
        self._add_import(from_section, import_type, name_import, typing)

    def add_import(self, name_import: str, import_type: ImportType, typing: bool = False) -> None:
        # Implementation detail: a regular import is just a "from" with no from
        self._add_import(name_import, import_type, None, typing)

    @property
    def imports(self) -> Dict[bool, Dict[ImportType, Dict[str, Set[Optional[str]]]]]:
        return self._imports

    def merge(self, file_import: "FileImport") -> None:
        """Merge the given file import format."""
        for typing, import_type_dict in file_import.imports.items():
            for import_type, package_list in import_type_dict.items():
                for package_name, module_list in package_list.items():
                    for module_name in module_list:
                        self._add_import(package_name, import_type, module_name, typing)
