        ...
        if len(builder.body_kwarg_name_to_content_types) == 1:
            retval.extend(
                self._set_body_content_kwarg(
                    builder, builder.parameters.body[0],
                    list(builder.body_kwarg_name_to_content_types.keys())[0]
                )
            )
        else:
            # we know we have one json input and one content input
            # first, we check if the user explicitly passed in content_type. If so, we route to the correct kwarg
            ...
            for idx, body_kwarg_name in enumerate(builder.body_kwarg_name_to_content_types):
                if body_kwarg_name == "json":
                    ...
                    content_types_container: ContentTypesContainer = builder.body_kwarg_name_to_content_types[
                        body_kwarg_name
                    ]
            ...
            if len(builder.schema_requests) == 1:
                ...
                content_types_container = builder.body_kwarg_name_to_content_types[chosen_kwarg]
                ...
                default_content_type = builder.body_kwarg_name_to_content_types["json"].default_content_type
                ...
                retval.append("    else:")
                default_content_type = builder.body_kwarg_name_to_content_types["content"].default_content_type
                retval.append(f'        content_type = content_type or "{default_content_type}"')
                retval.extend([
                    "        " + line for line in self._set_body_content_kwarg(builder, content_param, "content")
                ])
        return retval
