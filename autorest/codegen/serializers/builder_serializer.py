...
class _OperationSerializer(
    ...
    def _initialize_overloads(self, builder: OperationType) -> List[str]:
        retval: List[str] = []
        same_content_type = len(set(
            o.parameters.body_parameter.default_content_type for o in builder.overloads
        )) == 1
        if same_content_type:
            default_content_type = builder.overloads[0].parameters.body_parameter.default_content_type
            retval.append(
                f'content_type = content_type or "{default_content_type}"'
            )
