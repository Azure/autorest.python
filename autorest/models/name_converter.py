...
class NameConverter:
    ...
    def convert_yaml_names(self, yaml_code):
        yaml_code['info']['title'] = NameConverter._to_valid_python_name(yaml_code['info']['title'])
        # convert schemas
        for obj in yaml_code['schemas']['objects']:
            if self._seen_id(id(obj)):
                continue
            self._convert_object_schema(obj)
        for type_list, schemas in yaml_code['schemas'].items():
            for schema in schemas:
                ...
                if type_list == 'arrays' or type_list == 'arrays':
                    NameConverter._convert_schema_name(schema)
                    NameConverter._convert_schema_name(schema['elementType'])
                elif type_list == 'constants':
                    NameConverter._convert_schema_name(schema)
                    NameConverter._convert_schema_name(schema['value'])
                    NameConverter._convert_schema_name(schema['valueType'])
    ...
    def _convert_enum_schema(self, schema):
        ...
        NameConverter._convert_schema_name(schema['choiceType'])

        for choice in schema['choices']:
            if self._seen_id(id(choice)):
                continue
            NameConverter._convert_schema_name(choice, pad_string='Enum')
    ...
    def _convert_object_schema(self, schema):
        ...
        for prop in schema.get('properties', []):
            if self._seen_id(id(prop)):
                continue
            NameConverter._convert_schema_name(schema=prop, pad_string='Property')

    @staticmethod
    def _convert_schema_name(schema, **kwargs):
        schema['language']['python'] = dict(schema['language']['default'])
        schema_name = schema['language']['default']['name']
        schema_python_name = schema['language']['python']['name']
