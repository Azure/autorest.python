...
def get_pyproject_section(package_path: str, section_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "project" in data and "name" in data["project"]:
            ...
            assert actual_name == expected_name, f"Project name '{actual_name}' in pyproject.toml does not match folder name '{expected_name}'"
        ...
        for part in section_parts:
            assert part in current_data, f"pyproject.toml does not contain [{'.'.join(section_parts[:section_parts.index(part)+1])}] section"
...
def test_azure_sdk_build():
    ...
    test_paths = [
        "../../unbranded/generated/authentication-union",
        "../../azure/generated/authentication-union"
    ]
    ...
    for package_path in test_paths:
        ...
        assert "pyright" in azure_sdk_build, f"[tool.azure-sdk-build] section does not contain 'pyright' setting in {package_path}"
        assert azure_sdk_build["pyright"] is False, f"Expected pyright = false, but got pyright = {azure_sdk_build['pyright']} in {package_path}"
