...
class MediaTypesClientOperationsMixin(MediaTypesClientMixinABC):
    ...
    def analyze_body(self, input: Optional[Union[_models.SourcePath, IO]] = None, **kwargs: Any) -> str:
        ...
        if isinstance(input, (_serialization.Model, dict)):
            ...
            if not content_type:
                raise TypeError(
                    "Missing required keyword-only argument: content_type. Known values are: 'image/png', 'application/json', 'image/tiff', 'application/pdf', 'image/jpeg'"
                )
        else:
    ...
    @overload
    def body_three_types(self, message: JSON, *, content_type: str = "application/json", **kwargs: Any) -> str:
    ...
    @overload
    def body_three_types(self, message: str, *, content_type: str = "text/plain", **kwargs: Any) -> str:
        """Body with three types. Can be stream, string, or JSON. Pass in string 'hello, world' with
        content type 'text/plain', {'hello': world'} with content type 'application/json' and a byte
        string for 'application/octet-stream'.
    ...
    @distributed_trace
    def body_three_types(self, message: Union[JSON, IO, str], **kwargs: Any) -> str:
        """Body with three types. Can be stream, string, or JSON. Pass in string 'hello, world' with
        content type 'text/plain', {'hello': world'} with content type 'application/json' and a byte
        string for 'application/octet-stream'.

        :param message: The payload body. Is one of the following types: JSON, IO, str Required.
        :type message: JSON or IO or str
        :keyword content_type: Body Parameter content-type. Known values are: 'application/json',
        ...
        if isinstance(message, MutableMapping):
            ...
            raise TypeError("unrecognized type for message")
