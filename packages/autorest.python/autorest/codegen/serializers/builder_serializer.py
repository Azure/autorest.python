...
class RequestBuilderSerializer(
    ...
    def response_docstring(self, builder: RequestBuilderType) -> List[str]:
        import_rest = builder.import_core_name(same_module_name="rest")
        response_str = (
            f":return: Returns an :class:`~{import_rest}.HttpRequest` that you will pass to the client's "
            + "`send_request` method. See https://aka.ms/azsdk/dpcodegen/python/send_request for how to "
            + "incorporate this response into your code flow."
        )
        rtype_str = f":rtype: ~{import_rest}.HttpRequest"
...
class _OperationSerializer(
    ...
    def response_docstring(self, builder: OperationType) -> List[str]:
        ...
        return [
            response_str,
            rtype_str,
            f":raises ~{builder.import_core_name(same_module_name='exceptions')}.HttpResponseError:",
        ]
