...
def _build_command_line(
    ...
    **kwargs,
    ...
    return "autorest " + AUTOREST_M4_VERSION + " ".join(flag_strings) + debug_str
...
def _multiapi_command_line(location, debug):
    cwd = os.getcwd()
    cmd = (
        f'autorest {AUTOREST_M4_VERSION} {location} --use=. --multiapi --output-artifact=code-model-v4-no-tags ' +
...
def regenerate_package_mode(c, debug=False, swagger_group=None):
    ...
    cmds = [
        f'autorest {AUTOREST_M4_VERSION} {readme} --use=. --python-sdks-folder={cwd}/test/' for readme in package_mode
    ]
...
def regenerate_custom_poller_pager_legacy(c, debug=False):
    cwd = os.getcwd()
    cmd = (
        f'autorest {AUTOREST_M4_VERSION} test/azure/legacy/specification/custompollerpager/README.md --use=. --python-sdks-folder={cwd}/test/'
    )
...
def regenerate_mixed_api_version_legacy(c, debug=False):
    cwd = os.getcwd()
    cmd = (
        f'autorest {AUTOREST_M4_VERSION} test/azure/legacy/specification/mixedapiversion/README.md --use=. --python-sdks-folder={cwd}/test/'
    )
    _run_autorest([cmd], debug=debug)
...
def regenerate_custom_poller_pager_version_tolerant(c, debug=False):
    cwd = os.getcwd()
    cmd = (
        f'autorest {AUTOREST_M4_VERSION} test/azure/version-tolerant/specification/custompollerpager/README.md --use=. --python-sdks-folder={cwd}/test/'
    )
    _run_autorest([cmd], debug=debug)
...
def regenerate_samples(c, debug=False):
    ...
    for sample, special_flags in sample_to_special_flags.items():
        cmd = f'autorest {AUTOREST_M4_VERSION} samples/specification/{sample}/readme.md --use=.  '
        if special_flags:
            flag_strings = [
