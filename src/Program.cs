...
namespace AutoRest.Python
{
    public static class ExtensionsLoader
    {
        public static IAnyPlugin GetPlugin(string name)
        {
            switch (name)
            {
                case "Python": return new AutoRest.Python.PluginPy();
                case "Azure.Python": return new AutoRest.Python.Azure.PluginPya();
            }
    ...
    public class Program : NewPlugin
    {
        public static int Main(string[] args )
        {
            if(args != null && args.Length > 0 && args[0] == "--server") {
                ...
                connection.Dispatch<IEnumerable<string>>("GetPluginNames", async () => new []{ "python" });
