...
class FloatOperations:
    ...
    async def put(self, input: Optional[Union[float, "_models.FloatEnum"]] = None, **kwargs: Any) -> str:
        ...
        if input is not None:
            content = self._serialize.body(input, "float")
            content = json.dumps(content)
        else:
            content = None

        request = _rest.float.build_put_request(
    ...
    async def get(self, **kwargs: Any) -> Union[float, "_models.FloatEnum"]:
        ...
        request = _rest.float.build_get_request(template_url=self.get.metadata["url"], **kwargs)
        request.url = self._client.format_url(request.url)
        kwargs.pop("content_type", None)
